...
void rtp_state_sync_stream(
	...
	int _missing_rtp_frames = hdr_seq_number - expect_seq_number;
	int _missing_pcm_frames = expect_pcm_frames - rtp->ts_pcm_frames;

	if (_missing_rtp_frames != 0) {
		warn("Missing RTP packets [%u != %u]: %d",
				hdr_seq_number, expect_seq_number, _missing_rtp_frames);
		rtp->seq_number = hdr_seq_number;
	}

	if (_missing_pcm_frames != 0) {
		debug("Missing PCM frames [%u]: %d",
				hdr_timestamp, _missing_pcm_frames);
		rtp->ts_pcm_frames = expect_pcm_frames;
	}

	if (missing_rtp_frames != NULL)
		*missing_rtp_frames = _missing_rtp_frames;
	if (missing_pcm_frames != NULL)
		*missing_pcm_frames = _missing_pcm_frames;
